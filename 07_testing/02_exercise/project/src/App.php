...
class App
{
    ...
    public function run(): void
    {
        ...
        $messages = $this->getMessagesFromStorage($sessionStorage);
        $old = $this->getOldFromStorage($sessionStorage);
        $authUser = $this->getUserFromStorage($sessionStorage);
        ...
        if ($view) {
            ...
            foreach ($result->getEntries() as $entry) {
                ...
                if (isset(${$key})) {
                    $existingValue = ${$key};
                    Log::error("Variable with name '$key' is already defined and has value '$existingValue'.");
                } else {
                    ${$key} = $entry->getValue();
                }
            }
        ...
        $this->storeMessagesInStorage($sessionStorage, $messages);
        $this->storeOldInStorage($sessionStorage, $old);
    }
